...
function calculateHealthScore(analysisResults) {
  ...
  // Sprint 2: Score = 0.6*freshness + 0.4*coverage (per roadmap)
  ...
  const overallScore = Math.round((freshnessScore * freshnessWeight + coverageScore * coverageWeight) * 100);
  ...
  return {
    ...
    notes: 'Sprint 2: Score based on 60% freshness, 40% coverage. Future sprints will include validation.'
  };
}
