...
async function loadProtocols(dirPath, options = {}) {
  ...
  try {
    ...
    if (patterns) {
      ...
      searchPatterns = Object.values(PROTOCOL_PATTERNS).map(pattern => {
        // Extract the core pattern without anchors and flags
        let globPattern = pattern.source
          .replace(/^\^|\$$/g, '')  // Remove start/end anchors
          .replace(/\\\./g, '.')     // Unescape dots
          .replace(/.*\*/, '*');     // Simplify wildcards
